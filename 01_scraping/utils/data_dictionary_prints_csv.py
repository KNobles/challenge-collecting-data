...
import json
from multiprocessing import RLock, get_context, pool
class PropertyDataWriter:
    ...
    def write_data_to_file():
        file_location = os.path.join(pathlib.Path(__file__).parent.resolve(), "../../data", "urls.csv")
        with open(file_location, "r") as f:
            with get_context("fork").Pool() as pool:
                res = dict(pool.map(PropertyDataWriter.append_to_dict, f.readline()))
        data = pd.DataFrame(res, columns=PropertyDataWriter.columns)
        data.to_csv('Property_structured_threading.csv')
